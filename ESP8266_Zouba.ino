...
{  
  ...
  Serial.println(WiFi.localIP());
   String  dir  = " Votre Robot ZOUBA vous passe le salut et il vous  dit je suis dispo sur l'adress IP suivante :     " ;
  IPAddress broadCast = WiFi.localIP();
  char ip [26] ;
  sprintf( ip ,"%d.%d.%d.%d",broadCast[0],broadCast[1],broadCast[2],broadCast[3] ) ; 
  String message =  dir +ip ;
   Gsender *gsender = Gsender::Instance();    // Getting pointer to class instance
    String subject = " IP robot "
    if(gsender->Subject(subject)->Send("feresbengamra@gmail.com",message)) {
        Serial.println("Message send.");
    } else {
        Serial.print("Error sending message: ");
        Serial.println(gsender->getError());}w;;;;;;;;;;;;;;;;;;;wxwwwwwwww
